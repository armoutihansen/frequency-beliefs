...
artifacts committed), and revealed a structural finding: sharp identified
sets of distinct reports tile the simplex, so coverage under effective
misreporting is degenerately zero for every rule — the planned robustness
curves cannot exist. By author decision the simulation was dropped from
the paper; the tiling observation was added to §5.3 as a provable remark
(plus one incorrect adjacent clause fixed), strengthening both the
honesty of the optimal-reporting caveat and the case for the proposed
empirical coverage test. P2 is done: pgfplots figures (win-share
crossover; regret profiles) generated by `scripts/make_figures.py` from
the committed design-exercise outputs, integrated next to the tables in
§4. Paper compiles clean (34 pp, 0 errors, 0 undefined refs; three
pre-existing ~2pt overfull boxes noted for final polish). Remaining
before submission: final polish + the focused external human read.

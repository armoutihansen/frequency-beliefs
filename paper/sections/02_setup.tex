...
\]
Alternatively, for probability implementation, a bounded score can be normalized into a probability \(q(r,\omega)\in[0,1]\) of receiving a fixed prize.
Section 6 returns to this implementation issue.
...
\begin{definition}[Inverse belief region]
For an observed report \(r\in\OmegaN\), the belief region induced by \(S\) is
\[
P_S(r)
=
...
\]
It also reports exact-match payment probabilities when these quantities help compare implementation tradeoffs.

The next section applies this setup to quadratic, discrete-metric, Manhattan, Hamming, and Chebyshev frequency-report scoring rules.
